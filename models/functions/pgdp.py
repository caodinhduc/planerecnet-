...
class Plane_propagated_depth_guider(nn.Module):
    ...
    def measure_distance(self, a, b, d, pred_pointcloud):
        # 88283 x 3
        point = torch.column_stack((pred_pointcloud[:, :, 0].reshape(-1), pred_pointcloud[:, :, 1].reshape(-1), torch.zeros((480*640), dtype=float), torch.ones((480*640), dtype=float)))
        t = torch.tensor([a, b, -1.0, d])
    ...
    def forward(self, pred_depth, gt_masks, gt_depth, k_maritix):
        ...
        # check = torch.sum(output.requires_grad)
        # cumm = 0
        # for i in range(480):
        #     for j in range(640):
        #         if output[i, j].requires_grad == True:
        #             cumm += 1
